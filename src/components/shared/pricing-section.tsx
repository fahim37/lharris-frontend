...
export default function PricingSection() {
  ...
  const { data: plans } = useQuery({
    queryKey: ["plans"],
    queryFn: async () => {
      const res = await fetch(`https://lharris.onrender.com/api/v1/plans/get-all-plans`)
      return res.json()
    },
  ...
  const handleCreatePaymentIntent = async (planId: string, price: number) => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/payments/payment-intent`,
      {
        method: "POST",
        headers: {
              ...
              <CarouselItem key={plan._id} className="md:basis-1/2 lg:basis-1/3 pl-4">
                <Card className="h-full group bg-[#FFFFFF1A] hover:bg-[linear-gradient(170.72deg,_#F7E39F_-8.75%,_#091057_110.07%)] hover:text-[#091057] text-[#F7E39F] border-0 overflow-hidden py-10">
                  ...
                  <CardFooter className="p-6 pt-0">
                    <Button
                      onClick={() => handleCreatePaymentIntent(plan._id, plan.price)}
